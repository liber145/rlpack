...
def policy_fn(obs):
    ...
    x = tf.layers.dense(x, units=env.action_space.n, activation=tf.nn.softmax)
    return x
...
def run_main():
    agent = TD3(obs_fn=obs_fn,
                policy_fn=policy_fn,
                value_fn=value_fn,
                dim_act=env.action_space.n,
                log_freq=10,
                save_path=f"./log/td3_ramatari/{args.env}",
                policy_lr=1e-4,
                value_lr=1e-4,
                train_epoch=1)
    mem = Memory(capacity=int(1e5), dim_obs=(128, 4), dim_act=env.action_space.n)
    sw = SummaryWriter(log_dir=f"./log/td3_ramatari/{args.env}")
    totrew, totlen = 0, 0
    ...
    for i in tqdm(range(args.niter)):
        ...
        if d is True:
            s = env.reset()
            sw.add_scalars("td3", {"totrew": totrew, "totlen": totlen}, i)

...
# This is from https://github.com/apache/incubator-mxnet/blob/master/python/mxnet/log.py.
# -*- coding: utf-8 -*-
...
class _Formatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        fmt += '%(asctime)s %(process)d %(filename)s:%(funcName)s:%(lineno)d'
        fmt += ']\x1b[0m'
        fmt += ' %(message)s'
        if PY3:
...
def get_logger(name=None, filename=None, filemode=None, level=WARNING):
    ...
    if name is not None and not getattr(logger, '_init_done', None):
        ...
        if filename:
            ...
            hdlr = logging.StreamHandler()  # pylint: disable=E0012, redefined-variable-type
...
logger = get_logger("log", level=logging.DEBUG)

...
class DQN(TFEstimator):
    ...
    def __init__(self, config):
        ...
        # ------------- 搭建网络 --------------
        self.observation = tf.placeholder(shape=[None, self.dim_ob], dtype=tf.float32, name="observation")
        ...
        with tf.variable_scope("qnet"):
            ...
            self.qvals = tf.layers.dense(x, self.n_act, activation=None, trainable=True)
        ...
        gather_indices = tf.range(batch_size) * self.n_act + self.action
        ...
        self.update_target_op = update_ops

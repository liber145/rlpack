...
def run_main():
    agent = PG(obs_fn=obs_fn,
               policy_fn=policy_fn,
               dim_act=env.action_space.n,
               train_epoch=10,
               batch_size=args.batchsize,
               log_freq=10,
               save_path=f"./log/pg_ramatari/{args.env}",
               save_model_freq=1000)
    sw = SummaryWriter(log_dir=f"./log/pg_ramatari/{args.env}")
    totrew = 0
    for i in tqdm(range(args.niter)):
        ...
        sw.add_scalars("pg", {"totrew": np.mean(totrew_list)}, i)

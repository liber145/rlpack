...
class PPO(Base):
    ...
    def _build_algorithm(self):
        ...
        if self._is_action_continuous:
            ...
            policy_optimizer = tf.train.AdamOptimizer(self._lr)
            ...
            logp_old = tf.stop_gradient(gaussian_likelihood(self.action, self.old_mu, self.old_log_std))
            ...
            # policy_grads, policy_grad_norm = tf.clip_by_global_norm(policy_grads, self._max_grad_norm)
            # self._train_policy_op = policy_optimizer.apply_gradients(zip(policy_grads, policy_variables))

            # value_variables = tf.trainable_variables("main/value")
            # value_grads = tf.gradients(value_loss, value_variables)
            # value_grads, value_grad_norm = tf.clip_by_global_norm(value_grads, self._max_grad_norm)
            # self._train_value_op = optimizer.apply_gradients(zip(value_grads, value_variables))

            self._train_policy_op = policy_optimizer.minimize(policy_loss)
            self._train_value_op = optimizer.minimize(value_loss)

            policy_grad_norm = tf.global_norm(tf.gradients(policy_loss, tf.trainable_variables("main/policy")))
            value_grad_norm = tf.global_norm(tf.gradients(value_loss, tf.trainable_variables("main/value")))

            self._log_op = {"policy_loss": policy_loss, "value_loss": value_loss, "total_loss": total_loss,
                            "ratio": tf.reduce_mean(ratio), "policy_grad_norm": policy_grad_norm, "value_grad_norm": value_grad_norm,
                            "entropy": sample_entropy}

            # Build action sample.
            self.sample_action = self.mu + tf.exp(self.log_std) * tf.random_normal(shape=tf.shape(self.mu), dtype=tf.float32)
        ...
        if self._is_action_continuous:
            ...
            for _ in range(self._train_epoch):
                ...
                for i in range(math.ceil(n_sample / self._batch_size)):
                    ...
                    self.sess.run(self._train_policy_op,
                                  feed_dict={
            ...
            for _ in range(self._train_epoch):
                ...
                for i in range(math.ceil(n_sample / self._batch_size)):
                    ...
                    self.sess.run(self._train_value_op,
                                  feed_dict={
                                      self._observation: s_batch[span_index, ...],
                                      self.action: a_batch[span_index, ...],
                                      self._target_state_value: tsv_batch[span_index],
    ...
    def _parse_trajectory(self, trajectory):
        ...
        for i, (_, _, r) in enumerate(reversed(trajectory)):   # 注意这里是倒序操作。
            ...
            # target_sv_batch[i] = target_sv_batch[i+1] + r * self._discount
            last_state_value = state_value

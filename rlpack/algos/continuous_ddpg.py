...
class DDPG(Base):
    ...
    def build_network(self):
        ...
        # Build placeholders.
        self.observation_ph = tf.placeholder(tf.float32, [None, *self._dim_obs], "observation")
        self.action_ph = tf.placeholder(tf.float32, (None, self._dim_act), "action")
        ...
        with tf.variable_scope("act_net"):
            x = tf.layers.dense(self.observation_ph, 64, activation=tf.nn.relu, trainable=True)
            x = tf.layers.dense(x, 64, activation=tf.nn.relu, trainable=True)
            self.action = tf.layers.dense(x, self._dim_act, activation=tf.nn.tanh, trainable=True)

        with tf.variable_scope("dummy_act_net"):
            x = tf.layers.dense(self.observation_ph, 64, activation=tf.nn.relu, trainable=False)
            x = tf.layers.dense(x, 64, activation=tf.nn.relu, trainable=False)
            self.dummy_action = tf.layers.dense(x, self._dim_act, activation=tf.nn.tanh, trainable=False)

    def build_algorithm(self):
        """Build networks for algorithm."""
        self.optimizer = tf.train.AdamOptimizer(self._policy_lr)
        self.critic_optimizer = tf.train.AdamOptimizer(self._value_lr)
        self.target_qval_ph = tf.placeholder(tf.float32, (None,), "next_state_qval")
        self.grad_q_act_ph = tf.placeholder(tf.float32, (None, self._dim_act), "grad_q_act")
    ...
    def update(self, minibatch, update_ratio=None):
        ...
        for i in range(n_env):
            ...
            target_qval = r_batch[i, :] + self._discount * (1 - d_batch[i, :]) * qval

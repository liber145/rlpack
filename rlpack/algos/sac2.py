...
class SAC2(Base):
    ...
    def build_algorithm(self):
        ...
        with tf.control_dependencies([self.update_policy]):
            self.update_value = value_optimizer.minimize(self.value_loss, var_list=self.get_vars("main/q")+self.get_vars("main/v"))

        with tf.control_dependencies([self.update_value]):
            self.update_target = tf.group([tf.assign(v_targ, 0.995*v_targ + (1-0.995)*v_main) for v_main, v_targ in zip(self.get_vars("main"), self.get_vars("target"))])

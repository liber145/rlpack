...
def policy_fn(obs):
    ...
    x = tf.layers.dense(x, units=env.action_space.n * N_HISTOGRAM)
    return x


def run_main():
    agent = DistDQN(obs_fn=obs_fn,
                    policy_fn=policy_fn,
                    dim_act=env.action_space.n,
                    update_target_freq=100,
                    log_freq=10,
                    save_path=f"./log/distdqn_ramatari/{args.env}",
                    lr=1e-4,
                    n_histogram=N_HISTOGRAM,
                    train_epoch=1)
    mem = Memory(capacity=int(1e6), dim_obs=(128, 4), dim_act=env.action_space.n)
    sw = SummaryWriter(log_dir=f"./log/distdqn_ramatari/{args.env}")
    totrew, totlen = 0, 0
    ...
    for i in tqdm(range(args.niter)):
        ...
        if d is True:
            s = env.reset()
            sw.add_scalars("distdqn", {"totrew": totrew, "totlen": totlen}, i)

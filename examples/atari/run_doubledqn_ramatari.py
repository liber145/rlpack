...
def run_main():
    agent = DoubleDQN(obs_fn=obs_fn,
                      value_fn=value_fn,
                      dim_act=env.action_space.n,
                      update_target_freq=100,
                      log_freq=10,
                      save_path=f"./log/doubledqn_ramatari/{args.env}",
                      lr=1e-4,
                      train_epoch=1)
    mem = Memory(capacity=int(1e5), dim_obs=(128, 4), dim_act=env.action_space.n)
    sw = SummaryWriter(log_dir=f"./log/doubledqn_ramatari/{args.env}")
    totrew, totlen = 0, 0
    ...
    for i in tqdm(range(args.niter)):
        ...
        if d is True:
            s = env.reset()
            sw.add_scalars("doubledqn", {"totrew": totrew, "totlen": totlen}, i)

...
class Discriminator(Base):
    def __init__(self, save_path, obs_dims, n_actions):
        ...
        with tf.variable_scope('discriminator'):
            ...
            self.expert_s = tf.placeholder(dtype=tf.float32, shape=[None, obs_dims])
            self.expert_a = tf.placeholder(dtype=tf.int32, shape=[None])
            expert_a_one_hot = tf.one_hot(self.expert_a, depth=n_actions)
            ...
            self.agent_s = tf.placeholder(dtype=tf.float32, shape=[None, obs_dims])
            self.agent_a = tf.placeholder(dtype=tf.int32, shape=[None])
            agent_a_one_hot = tf.one_hot(self.agent_a, depth=n_actions)
            # add noise for stabilise training

...
class SAC2(Base):
    ...
    def build_network(self):
        ...
        with tf.variable_scope("pi"):
            ...
            self.mu = tf.layers.dense(x, units=self.dim_action, activation=tf.tanh)
            ...
            normal_dist = tf.contrib.distributions.MultivariateNormalDiag(loc=self.mu, scale=tf.exp(self.log_std))
        ...
        with tf.variable_scope("q1"):
            ...
            x = tf.layers.dense(x, units=100, activation=tf.nn.relu)
            x = tf.layers.dense(x, units=100, activation=tf.nn.relu)
            self.q1 = tf.layers.dense(x, units=1)

        with tf.variable_scope("q1", reuse=True):
            x = tf.concat([self.observation, self.pi], axis=1)
            x = tf.layers.dense(x, units=100, activation=tf.nn.relu)
            x = tf.layers.dense(x, units=100, activation=tf.nn.relu)
            self.q1_pi = tf.layers.dense(x, units=1)

        with tf.variable_scope("q2"):
            x = tf.concat([self.observation, self.action], axis=1)
            x = tf.layers.dense(x, units=100, activation=tf.nn.relu)
            x = tf.layers.dense(x, units=100, activation=tf.nn.relu)
            self.q2 = tf.layers.dense(x, units=1)

        with tf.variable_scope("q2", reuse=True):
            x = tf.concat([self.observation, self.pi], axis=1)
            x = tf.layers.dense(x, units=100, activation=tf.nn.relu)
            x = tf.layers.dense(x, units=100, activation=tf.nn.relu)
            self.q2_pi = tf.layers.dense(x, units=1)

        with tf.variable_scope("v"):
            x = tf.layers.dense(self.observation, units=100, activation=tf.nn.relu)
            x = tf.layers.dense(x, units=100, activation=tf.nn.relu)
            self.v = tf.layers.dense(x, units=1)

        with tf.variable_scope("v_targ"):
            x = tf.layers.dense(self.next_observation, units=100, activation=tf.nn.relu)
            x = tf.layers.dense(x, units=100, activation=tf.nn.relu)
            self.v_targ = tf.layers.dense(x, units=1)
    ...
    def build_algorithm(self):
        ...
        self.update_policy = pi_optimizer.minimize(self.pi_loss, var_list=self.trainable_variables("pi"))

        # Train value.
        value_optimizer = tf.train.AdamOptimizer(learning_rate=1e-3)
        value_vars = tf.trainable_variables("q1") + tf.trainable_variables("q2") + tf.trainable_variables("v")
        ...
        main_vars = tf.trainable_variables("v")
        target_vars = tf.trainable_variables("v_targ")
        ...
        print("main vars:", len(self.get_vars("main")))
        print("target vars:", len(self.get_vars("target")))
        self.init_target = tf.group([tf.assign(v_targ, v_main) for v_main, v_targ in zip(self.get_vars("main"), self.get_vars("target"))])

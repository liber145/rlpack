...
class TD3(Base):
    def __init__(self,
        ...
        self._action_low = np.array([-1.0 for i in range(self._dim_act)])
        self._action_high = np.array([1.0 for i in range(self._dim_act)])
    ...
    def build_network(self):
        ...
        self._action = tf.placeholder(tf.float32, [None, self._dim_act], name="action")
        ...
        with tf.variable_scope("main/policy"):
            ...
            self._p_act = tf.layers.dense(x, self._dim_act)

        with tf.variable_scope("main/value_1"):
            x = self._dense(self._observation)
            self._qval_1 = tf.squeeze(tf.layers.dense(x, 1))

        with tf.variable_scope("main/value_2"):
            x = self._dense(self._observation)
            self._qval_2 = tf.squeeze(tf.layers.dense(x, 1))

        with tf.variable_scope("policy/policy"):
            x = self._dense(self._observation)
            self._target_p_act = tf.layers.dense(x, self._dim_act)

        with tf.variable_scope("policy/value_1"):
            x = self._dense(self._observation)
            self._target_qval_1 = tf.squeeze(tf.layers.dense(x, 1))

        with tf.variable_scope("policy/value_2"):
            x = self._dense(self._observation)
            self._target_qval_2 = tf.squeeze(tf.layers.dense(x, 1))
    ...
    def _update_target(self, net1, net2, rho=0):
        ...
        params2 = tf.global_variables(net2)
    ...
    def build_algorithm(self):
        ...
        self.update_target_policy_op = self._update_target("target_policy_net", "policy_net", self._target_update_ratio)
        self.update_target_value_op = self._update_target("target_value_net", "value_net", self._target_update_ratio)
    ...
    def get_action(self, obs):
        ...
        action = self.sess.run(self.act, feed_dict={self._observation: newobs})
    ...
    def update(self, minibatch, update_ratio):
        ...
        for i in range(n_env):
            ...
            next_a_batch = self.sess.run(self.target_act, feed_dict={self._observation: next_s_batch[i, :]}) + np.random.normal(scale=self.noise_std, size=(batch_size, self._dim_act))
            assert next_a_batch.shape == (batch_size, self._dim_act)
            next_a_batch = np.clip(next_a_batch, self._action_low, self._action_high)
            q1, q2 = self.sess.run([self.target_qval_1, self.target_qval_2], feed_dict={self._observation: next_s_batch[i, :], self._action: next_a_batch})
            assert q1.shape == (batch_size,) and q2.shape == (batch_size,)
            target_qval = r_batch[i, :] + self._discount * (1 - d_batch[i, :]) * np.minimum(q1, q2)
        ...
        self.sess.run(self.train_critic_op, feed_dict={
            self._observation: mb_s,
            self._action: mb_a,
            self.target_qval: mb_target,
            self.critic_lr: self._value_lr})

        global_step, _ = self.sess.run([tf.train.get_global_step(), self.increment_global_step])

        # Update actor net.
        if global_step % self._policy_decay == 0:
            self.sess.run(self.train_actor_op, feed_dict={self._observation: mb_s, self.actor_lr: self._policy_lr})
